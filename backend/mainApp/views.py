...
class CartViewSet(viewsets.ViewSet):
    ...
    def get_cart(self, request):
        if request.user.is_authenticated:
            ...
            # Always ensure session is initialized and consistent
            if not request.session.session_key:
                request.session.create()  # <-- This is more explicit and reliable than save()

            session_key = request.session.session_key
            print('seeion:',session_key)
...
def merge_guest_cart_to_user(sender, request, user, **kwargs):
    session_key = request.session.session_key
    if not session_key:
        return
    ...
    for item in guest_cart.items.all():
        existing, created = CartItem.objects.get_or_create(cart=user_cart, product=item.product)
        if not created:
...
from rest_framework.response import Response
from rest_framework_simplejwt.tokens import RefreshToken
from .serializers import RegisterAPI
